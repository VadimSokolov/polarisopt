...
class Calibrate_NN(ABC):
    ...
    def set_base(self, problem_info):
...
class Calibrate_DRNN(Calibrate_NN):
    ...
    Examples::
        ...
        >>> c.run(problem_info, num_trials=20, num_rec_points=3)
    """
    ...
    def run(self,problem_info,num_grid_points=2000,num_trials=30,num_rec_points=3, acq_type='EI', pr=False):
        pinfo = self.set_base(problem_info)
        ...
        for l in range(0,num_trials+1):
            ...
            util.thread_it(self.eval_loss,[(problem_info,row,pr) for row in pend_samples])
            if l<num_trials:
    ...
    def eval_loss(self,problem_info, variables, pr=False):
        ...
        _, _, _, NN_v, _ = archiver.load_DR_settings(problem_info.settings_filename)
            #[epochs,learning_rate,lambda,penalty,XDR_layer,DRX_layer,DRY_layer]
        NN_var=[
            problem_info.dim_in,
            problem_info.dim_out,
            epochs,
            lr,
            *NN_v[2:4],
            *layers
        ...
        tDR_model = dim_red.NN_method(dim_DR,problem_info.orig_range,NN_var) #orig range = not the Nn inputs
        #NN_var = [dim_in,dim_out,epochs,learning_rate,lambda,penalty,XDR_layer,DRX_layer,DRY_layer]
        train_X,train_Y = problem_info.load_training()
        tDR_model.calculate(train_X,train_Y,pr=pr)
        ...
        if pr:
            print(y)
        archiver.replace_pend(variables,y,self.res_filename)
...
class Calibrate_Mean_NN(Calibrate_NN):
    ...
    def run(self, problem_info, DR_model, num_grid_points=2000, num_trials=30, num_rec_points=3, acq_type='EI', pr=False):
        pinfo = self.set_base(problem_info)
        pinfo.set_attribute('num_grid_points',num_grid_points)
        pinfo.set_attribute('num_rec_points',num_rec_points)
        pinfo.set_attribute('acq_type',acq_type)
        ...
        for l in range(0,num_trials+1):
            #After a Bayes set is recorded, we need to evaluate the pending ones denoted with 'P'
            _,pend_samples = archiver.import_dataset(self.res_filename)
            util.thread_it(self.eval_loss, [(problem_info, DR_model, row, pr) for row in pend_samples])
            if l<num_trials:
            #If less then the number of trials we run, run another Bayes set
                print("running loop number %d of %d" % (l+1,num_trials))
                bo.main_loop(pinfo)
    ...
    def eval_loss(self,problem_info, DR_model, variables, pr=False):
        ...
        mean_model.calculate(problem_info,pr=pr)
        train_X,train_Y = problem_info.load_training()
        X_0 = transforms.normalize(train_X, problem_info.orig_range[0])
        inputs,labels=torch.as_tensor(X_0),torch.as_tensor(train_Y)
