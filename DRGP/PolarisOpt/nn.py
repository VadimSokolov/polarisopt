...
class Custom_Loss(torch.nn.Module):
    ...
    def forward(self, x_hat, y_hat, train_X, train_Y, xlb, xub, quiet = True):
...
def train_NN(train_X, train_Y, epochs, network, quiet = True):
    ...
    for epoch in range(int(epochs)):
        for batch_idx, (inputs, labels) in enumerate(train_loader):
            ...
            if not quiet and batch_idx == 0 and epoch%100 == 0: 
                print("loss: %s"%(loss.item()))
    network.eval()



def train_DRNN(train_X, train_Y, epochs, network, x_range, quiet = True):
    #will use roughly 10% of the training values as a batch for a max of 256; if 10% is less than 2, we don't batch
    # Find optimal model hyperparameters through batching
    if len(train_X)*.1>= 2:
        num_batch = min(2**int(math.log(len(train_X)*.1, 2)), 256)
