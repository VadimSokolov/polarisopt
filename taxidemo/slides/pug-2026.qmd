...
::: {.fragment}
polarisopt injects each sampled value into the named JSON before every run (`type` is `float` by default, or `int`). Adding a knob is **one line here**, no code. Want it in the study file instead? Drop the same list under `inline:` (what the taxi demo does).
:::
...
::: {.columns}
::: {.column width="50%"}
- **Resume** — `Ctrl-C` the master, `polarisopt resume`; RNG + surrogate restored, and on-disk results are harvested even if Slurm's status went stale (v0.11).
...
no breaking changes through **v0.11**.
:::
